...
async def get_user_bans(
    db: DbDep,
    current_user: CurrentUserDep,
    user_telegram_id: int | None = Query(
        None,
        description="Filter by user telegram ID",
    ),
    ...
    """Fetch bans. Only superusers can view bans."""
    if not current_user.is_superuser:
...
async def get_ban(db: DbDep, current_user: CurrentUserDep, ban_id: int):
    """Fetch a specific ban by ID. Only superusers can view bans."""
    if not current_user.is_superuser:
        raise HTTPException(status_code=403, detail="Only superusers can view bans")
...
async def check_user_ban_status(
    ...
    """Check if a user is currently banned. Only superusers can check ban status."""
    if not current_user.is_superuser:
        raise HTTPException(
            status_code=403,
            detail="Only superusers can check ban status",
        )
...
async def update_user_ban(
    ...
    current_user: CurrentUserDep,
    ban_id: int,
    update_data: BanUpdateSchema,
):
    """Update a ban. Only superusers can update bans."""
    if not current_user.is_superuser:
...
async def delete_user_ban(db: DbDep, current_user: CurrentUserDep, ban_id: int):
    """Delete a ban (unbans the user). Only superusers can delete bans."""
    if not current_user.is_superuser:
