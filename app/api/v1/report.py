...
async def create_user_report(
    db: DbDep, current_user: CurrentUserDep, report_data: ReportInSchema
):
...
async def get_report(db: DbDep, current_user: CurrentUserDep, report_id: int):
    ...
    if not current_user.is_superuser and (not report or report.from_user_id != current_user.id):
...
async def update_report(
    db: DbDep, current_user: CurrentUserDep, report_id: int, update_data: ReportUpdateSchema
):
    """Updates a report's status. Only superusers can update reports."""
    if not current_user.is_superuser:
        raise HTTPException(status_code=403, detail="Only superusers can update reports")
...
async def delete_user_report(db: DbDep, current_user: CurrentUserDep, report_id: int):
    ...
    if not current_user.is_superuser:
        raise HTTPException(status_code=403, detail="Only superusers can delete reports")
